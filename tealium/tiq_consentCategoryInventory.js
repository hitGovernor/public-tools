...
utui.automator.getAllProfiles(utui.login.account).then(function (profiles) {
  ...
  PROFILES.forEach(function (profile, idx) {
    ...
    utui.profile.getProfile(null, {
      ...
      PROCESSED_PROFILES++;
      console.log(`⮑ ${PROCESSED_PROFILES} of ${PROFILES.length} profiles processed - Profile: ${profile}`);
  ...
  setTimeout(function () {
    if (PROCESSED_PROFILES < PROFILES.length) {
      console.warn(`⧰ Timeout reached. Only ${PROCESSED_PROFILES} of ${PROFILES.length} profiles processed. Downloading partial results...`);
      downloadCSV();
    }
  ...
  function downloadCSV() {
    ...
    a.href = url;
    a.download = `tealium_tag_audit_${utui.login.account}_consolidated_${new Date().toISOString().slice(0, 10)}.csv`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    window.URL.revokeObjectURL(url);

    console.log(`\n💾 CSV file downloaded: tealium_tag_audit_${utui.login.account}_consolidated_${new Date().toISOString().slice(0, 10)}.csv`);
